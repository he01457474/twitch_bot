...
- 有新的偏好規則或權限設定，都寫回 CLAUDE.md 和 AGENTS.md（兩個檔案保持同步）
...
- 私人秘書 `projects/secretary-bot/` 程式修改完成並通過檢查後，自動重啟私人秘書，讓執行中的 Bot 套用最新版本
...
- 既有 IRL 台主要更新台主包、整理說明或補 BRB / NOALBS 工具時，除非使用者明確要求「重新產生密鑰」，否則一律沿用 `relay_users.json` 裡既有的 publish / read 密鑰，不要改台主 Stream ID。
- 既有 IRL 台主更新 NOALBS / BRB 工具時，安裝器應優先讀取舊 `.env` 與 `config.json`，沿用 Twitch Bot Token、OBS WebSocket 密碼、場景名稱與中繼伺服器設定；只有缺資料時才要求台主重填。
- IRL 台主的 BRB 頻道由管理員端白名單工具設定後匯出台主包，台主包內帶 `brb-config.json`；不要要求台主手動編輯 JSON。
